...
public class ENSNameViewModel: ObservableObject {
    ...
    func fetch(address: String) {
        let json: [String: Any] = ["query": ENSNameViewModel.baseGQL, "variables": ["id": address.lowercased()], "operationName": "getNames"]
        ...
        networkRequest.httpBody = try! JSONSerialization.data(withJSONObject: json)

        apiManager.networkTask(request: networkRequest as URLRequest) { [weak self] (response: Result<TheGraphBaseResponseModel, Error>) in
            switch response {
            case .success(let data):
                let ownedNames = data.data.account.domains.filter { !$0.name.hasSuffix("addr.reverse") }.map { ENSNameViewModel.ensNameFromRawDomain($0, wrapped: false) }
                let wrappedNames = data.data.account.wrappedDomains.map { $0.domain }.map { ENSNameViewModel.ensNameFromRawDomain($0, wrapped: true) }
    ...
    private static func ensNameFromRawDomain(_ domain: RawDomainModel, wrapped: Bool) -> ENSName {
        return ENSName(
            name: domain.name,
            imageUrl: "https://metadata.ens.domains/mainnet/avatar/\(domain.name)",
